...
import type { VideoExerciseItem } from "@/lib/types";
...
export function workoutBlocks(v: {
  blocks?: { name: string; mins: number; items?: VideoExerciseItem[] }[];
  mins: number;
  format: string;
}): { name: string; mins: number; items: string[] }[] {
  // Authored blocks carry their exercise list; the synthesised fallback below has
  // none, so consumers must treat `items` as possibly empty rather than assume it.
  if (v.blocks?.length) {
    return v.blocks.map((b) => ({
      name: b.name,
      mins: b.mins,
      items: (b.items ?? []).map(exerciseName),
    }));
  }
  const warm = Math.max(2, Math.round(v.mins * 0.14));
  const cool = Math.max(2, Math.round(v.mins * 0.16));
  const main = v.mins - warm - cool;
  if (v.mins <= 15) return [{ name: "Bemelegítés", mins: warm, items: [] }, { name: v.format, mins: main, items: [] }, { name: "Levezetés", mins: cool, items: [] }];
  const m1 = Math.ceil(main / 2);
  return [
    { name: "Bemelegítés", mins: warm, items: [] },
    { name: `${v.format} - 1. blokk`, mins: m1, items: [] },
    { name: `${v.format} - 2. blokk`, mins: main - m1, items: [] },
    { name: "Levezetés", mins: cool, items: [] },
  ];
}
