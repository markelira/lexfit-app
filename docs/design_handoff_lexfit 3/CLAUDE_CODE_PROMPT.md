...
| `40-ONBOARDING.md` | Funnel — Onboarding, Auth & Checkout. |
| `41-ONBOARDING-DEV-PLAN.md` | **Build plan for the funnel** — nine phases. Read `40` first, build from `41`. |
...
7c. **Onboarding, Auth & Checkout** — its own nine-phase plan, `41-ONBOARDING-DEV-PLAN.md`. It **inverts the current order**: the app runs auth → onboarding, the spec runs onboarding → auth → checkout. Largest structural change in the bundle.
